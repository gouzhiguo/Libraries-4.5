...
namespace EC.Libraries.WeiXin
{
    ...
    public class WeiXinProvider
    {
        ...
        /// </summary>
        public CreateMenuResponse CreateMenu(string accessToken, string data)
        {
            ...
            var result = new HttpUtils().Post(requestUrl, data);
            if (!string.IsNullOrWhiteSpace(result))
            {
                response = result.ToObject<CreateMenuResponse>();
            }
            else
            {
                response.errmsg = "api请求失败！";
            }
            return response;
        }
        ...
        public void DeleteMenu(string access_token)
        {
            string requestUrl = "https://api.weixin.qq.com/cgi-bin/menu/delete?";
        ...
        {
            var orderNotifyImpl = new OrderNotifyImpl(null)
            {
                apiKey = request.apiKey
            };
        ...
        public string CreateTicket(string accessToken, string sceneStr)
        {
            var response = "";
        ...
        /// <summary>
        /// 使用微信模板发送固定消息 https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1433751277
        /// </summary>
        ...
        public SendTemplateMessageResponse SendTemplateMessage(SendTemplateMessageRequest request)
        {
            var sendTemplateMessageResponse = new SendTemplateMessageResponse();

            var requestUrl = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=" + request.AccessToken;

            var userInfoResponse = this.GetUserInfo(request.AccessToken, request.OpenId);

            switch (userInfoResponse.subscribe)
            {
                case 0:
                    break;
                default:
                    ...
                    var result = new HttpUtils().Post(requestUrl, templateMessage.ToJson());
                    if (!string.IsNullOrWhiteSpace(result))
                    {
                        sendTemplateMessageResponse = result.ToObject<SendTemplateMessageResponse>();
                    }
                    else
                    {
                        sendTemplateMessageResponse.errmsg = "api请求失败！";
                    }
                    break;
            }
