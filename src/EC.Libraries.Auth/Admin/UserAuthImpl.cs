...
namespace EC.Libraries.Auth
{
    ...
    public class UserAuthImpl : IUserAuth
    {
        /// <summary>
        /// 写用户认证
        /// </summary>
        ...
        public void SignIn(TicketUser user, bool createPersistentCookie)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public TicketUser GetAuthenticatedUser()
        {
//#if TEST
//            return new Customer
//            {
...
//#endif
            ...
            if (!HttpContext.Current.Request.IsAuthenticated || !(HttpContext.Current.User.Identity is FormsIdentity))
            {
                ...
                else
                {
                    ...
                    return JsonUtil.ToObject<TicketUser>(ticket.UserData);
                }
            }

            var formsIdentity = (FormsIdentity)HttpContext.Current.User.Identity;

            return JsonUtil.ToObject<TicketUser>(formsIdentity.Ticket.UserData);
        }
    }
}
